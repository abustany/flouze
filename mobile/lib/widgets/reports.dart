      ...
      children: [
              ...
              children: balance.entries.map((entry) {
                ...
                return TableRow(children: [
                  Padding(
                      ...
                      child: Text(personName(members, entry.key),
                          key: subkey(key, '-$i-name'),
                          textAlign: TextAlign.right)),
                  Padding(
                      padding: EdgeInsets.all(12.0),
                      child: Text(
                          '${amountToString(entry.value)} ${AppConfig.currencySymbol}',
                          key: subkey(key, '-$i-balance')))
                ]);
              }).toList(),
            )),
        if (transfers.isNotEmpty)
          Column(
            ...
            children: <Widget>[
              Padding(
                  child: Text(FlouzeLocalizations.of(context).howToSettle),
                  ...
                  ...transfers.map((entry) {
                    ...
                    return TableRow(children: [
                      Padding(
                          padding: EdgeInsets.all(12.0),
                          child: Text(
                              FlouzeLocalizations.of(context)
                                  .xOwesY(debitorName, creditorName),
                              key: subkey(key, '-$i-owes'),
                              textAlign: TextAlign.right)),
                      Padding(
                          padding: EdgeInsets.all(12.0),
                          child: Text(
                              '${amountToString(entry.amount.toInt())} ${AppConfig.currencySymbol}',
                              key: subkey(key, '-$i-amount')))
                    ]);
                  })
                ],
              )
