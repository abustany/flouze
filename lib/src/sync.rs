...
fn rebase_local_transactions(local: &mut Repository, remote: &Remote, account: &mut model::Account) -> errors::Result<()> {
    ...
    let remote_transactions = remote.get_child_transactions(&account.uuid, &account.latest_synchronized_transaction)?;

    if remote_transactions.is_empty() {
        return Err(errors::ErrorKind::InconsistentServerResponse.into());
    }

    check_transaction_id_uniqueness(local, &account.uuid, &remote_transactions.iter().map(|tx| &tx.uuid).collect::<Vec<&model::TransactionId>>())?;

    if !check_chain_consistency(&remote_transactions.iter().map(|tx| tx).collect::<Vec<&model::Transaction>>()) {
        return Err(errors::ErrorKind::InconsistentChain.into());
    }

    debug!("Received {} transactions from remote, rebasing local transactions", remote_transactions.len());

    let (rebased_transactions, old_transaction_ids) = rebase_transactions(local, &account, &remote_transactions.last().unwrap().uuid)?;
    let latest_id = rebased_transactions.last()
    ...
    for tx in rebased_transactions {
        debug!("Adding rebased transaction {} (parent: {})", model::IdAsHex(&tx.uuid), model::IdAsHex(&tx.parent));
        local.add_transaction(&account.uuid, &tx)?;
    }

    debug!("Setting latest transaction to {}", model::IdAsHex(&latest_id));
    debug!("Setting latest synchronized transaction to {}", model::IdAsHex(&latest_synchronized_id));
...
fn check_transaction_id_uniqueness(repo: &Repository, account_id: &model::AccountId, ids: &[&model::TransactionId]) -> errors::Result<()> {
    for id in ids {
        match repo.get_transaction(account_id, id) {
            ...
            Ok(_) => { return Err(errors::ErrorKind::DuplicateTransactionId(id.to_owned().to_vec()).into()); },
            Err(e) => { return Err(e); }
        }
    }
...
pub fn sync(local: &mut Repository, remote: &mut Remote, account_id: &model::AccountId) -> errors::Result<()> {
    ...
    debug!("{} transaction(s) to push to the remote", local_transactions.len());
    ...
    let local_transactions_refs: Vec<&model::Transaction> = local_transactions.iter().map(|tx| tx).collect();
...
fn rebase_transactions(repo: &Repository, account: &model::Account, new_base: &model::TransactionId) -> errors::Result<(Vec<model::Transaction>, Vec<model::TransactionId>)> {
    ...
    for mut tx in to_rebase {
        ...
        debug!("Rebasing transaction {} (old parent: {}) as {} on top of {}", model::IdAsHex(&tx.uuid), model::IdAsHex(&tx.parent), model::IdAsHex(&new_id), model::IdAsHex(&parent));
...
mod tests {
    ...
    fn test_sync_initial_pull() {
        ...
        remote_repo.set_latest_transaction(&account.uuid, &tx3.uuid).unwrap();
        ...
        assert_eq!(&remote.get_latest_transaction(&account.uuid).unwrap(), &tx3.uuid);
    }
    ...
    fn test_sync_initial_push() {
        ...
        local_repo.set_latest_transaction(&account.uuid, &tx3.uuid).unwrap();
    ...
    fn test_sync_append_transactions() {
        ...
        local_repo.add_transaction(&account.uuid, &tx2).unwrap();
        remote_repo.add_transaction(&account.uuid, &tx2).unwrap();
        remote_repo.set_latest_transaction(&account.uuid, &tx2.uuid).unwrap();
        local_repo.add_transaction(&account.uuid, &tx3).unwrap();
        local_repo.set_latest_transaction(&account.uuid, &tx3.uuid).unwrap();
        local_repo.set_latest_synchronized_transaction(&account.uuid, &tx2.uuid).unwrap();
    ...
    fn test_sync_rebase() {
        ...
        remote_repo.add_transaction(&account.uuid, &tx1).unwrap();
        remote_repo.add_transaction(&account.uuid, &tx3).unwrap();
        local_repo.set_latest_transaction(&account.uuid, &tx2.uuid).unwrap();
        remote_repo.set_latest_transaction(&account.uuid, &tx3.uuid).unwrap();
        local_repo.set_latest_synchronized_transaction(&account.uuid, &tx1.uuid).unwrap();
