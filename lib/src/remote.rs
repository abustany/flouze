...
pub mod tests {
    ...
    pub fn expect_inconsistent_chain<T: Debug>(r: errors::Result<T>) {
        match r {
            ...
            _ => {
                panic!("Expected InconsistentChain error, got {:?}", r);
            }
        }
    }
    ...
    pub fn expect_must_rebase<T: Debug>(r: errors::Result<T>) {
        match r {
            ...
            _ => {
                panic!("Expected MustRebase error, got {:?}", r);
            }
        }
    }
    ...
    pub fn test_initial_sync_to_remote_prepare_repo(
        remote_repo: &mut dyn Repository,
    ) -> model::Account {
    ...
    pub fn test_initial_sync_to_remote(remote: &mut dyn Remote, account_id: &model::AccountId) {
        assert_eq!(
            remote.get_latest_transaction(&account_id).unwrap(),
            model::INVALID_ID
        );
        assert_eq!(
            remote.get_child_transactions(&account_id, &vec!()).unwrap(),
            vec!()
        );
        ...
        remote
            .receive_transactions(account_id, &vec![&tx1, &tx2, &tx3])
            .unwrap();
        assert_eq!(
            &remote.get_latest_transaction(&account_id).unwrap(),
            &tx3.uuid
        ...
        assert_eq!(
            remote
                .get_child_transactions(account_id, &tx1.uuid)
                .unwrap(),
            vec!(tx2.clone(), tx3.clone())
        );
        assert_eq!(
            remote
                .get_child_transactions(account_id, &tx3.uuid)
                .unwrap(),
            vec!()
        );
    }
