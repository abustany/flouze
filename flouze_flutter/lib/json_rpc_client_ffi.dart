...
typedef flouze_json_rpc_client_destroy_t = ffi.Void Function(ffi.Pointer<NativeJsonRpcClient>);
...
class _JsonRpcClientBindings {
  ffi.Pointer<NativeJsonRpcClient> Function(ffi.Pointer<Utf8>, ffi.Pointer<ffi.Pointer<Utf8>>) create;
  ffi.Void Function(ffi.Pointer<NativeJsonRpcClient>) destroy;
  ffi.Void Function(ffi.Pointer<NativeJsonRpcClient>, ffi.Pointer<Byte>, int, ffi.Pointer<ffi.Pointer<Utf8>>) createAccount;
  ffi.Void Function(ffi.Pointer<NativeJsonRpcClient>, ffi.Pointer<Byte>, int, ffi.Pointer<ffi.Pointer<Byte>>, ffi.Pointer<ffi.IntPtr>, ffi.Pointer<ffi.Pointer<Utf8>>) getAccountInfo;
...
class _JsonRpcHelpers {
  static void destroy(ffi.Pointer<NativeJsonRpcClient> ptr) => _jsonRpcClientBindings.destroy(ptr);

  static void createAccount(ffi.Pointer<NativeJsonRpcClient> ptr, Account account) =>
      createAccountBytes(ptr, account.writeToBuffer());

  static void createAccountBytes(ffi.Pointer<NativeJsonRpcClient> ptr, Uint8List account) {
    withError((errPtr) =>
        withCBytes(account, (data, len) =>
            _jsonRpcClientBindings.createAccount(ptr, data, len, errPtr)));
  }

  static Account getAccountInfo(ffi.Pointer<NativeJsonRpcClient> ptr, List<int> accountId) =>
      Account.fromBuffer(getAccountInfoBytes(ptr, Uint8List.fromList(accountId)));

  static Uint8List getAccountInfoBytes(ffi.Pointer<NativeJsonRpcClient> ptr, Uint8List accountId) =>
    withOutCBytes((accountDataPtr, accountDataLen) =>
        withError((errPtr) =>
            withCBytes(
                accountId, (cAccountId, cAccountIdLen) =>
                _jsonRpcClientBindings.getAccountInfo(
                    ptr, cAccountId, cAccountIdLen, accountDataPtr,
                    accountDataLen, errPtr)))
    ).bytes ?? Uint8List(0);
}
...
class JsonRpcClient extends IsolateProxy {
  JsonRpcClient._(Future<SendPort> Function() factory)
      : super(factory);
  ...
  static void _isolateMain(dynamic params) {
    ...
    IsolateProxy.handleCalls(
      mainParams.sender,
          () {
        return withError((errPtr) => withCStr(mainParams.url, (cUrl) => _jsonRpcClientBindings.create(cUrl, errPtr)));
      },
      _handleCall,
    );
  }
  ...
  static dynamic _handleCall(dynamic instance, Call call) {
    ...
    switch(call.runtimeType) {
      ...
      case _JsonRpcClientCreateAccount:
        return _JsonRpcHelpers.createAccountBytes(ptr, (call as _JsonRpcClientCreateAccount).account);
      case _JsonRpcClientGetAccountInfo:
        return _JsonRpcHelpers.getAccountInfoBytes(ptr, (call as _JsonRpcClientGetAccountInfo).accountId);
  ...
  Future<Account> getAccountInfo(List<int> accountId) =>
      call(_JsonRpcClientGetAccountInfo(Uint8List.fromList(accountId))).then((bytes) =>
          Account.fromBuffer(bytes)
      );

  Future<ffi.Pointer<NativeJsonRpcClient>> getNativePointer() =>
      call(_JsonRpcClientGetNativePointer());
}